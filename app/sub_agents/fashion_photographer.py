from app.schema import Trend, TaxonomyAttributes, VisualAssets, MarketingAttributes, TargetAudienceProfile
from app.config import GEMINI_MODEL_NAME, IMAGEN_MODEL_NAME, PROJECT_ID, LOCATION, NANO_BANANA_PRO_MODEL_NAME, STANDARD_GENERATION_CONFIG
import logging
...
class FashionPhotographer:
    def __init__(self):
        try:
            with open("app/prompts/fashion_photographer.md", "r") as f:
                self.prompt_template = f.read()
                
        except Exception as e:
            logging.error(f"Failed to load fashion_photographer prompt: {e}")
    ...
    def generate_campaign_image(self, product_image_path: str, art_direction: str) -> str:
        ...
        try:
          ...
          #             if hasattr(part.inline_data.data, "image"):
          #                 # If it's PIL or bytes
          #                 with open(output_path, "wb") as f:
          #                     f.write(image_bytes)
          #             elif hasattr(part.inline_data.data, "image_bytes"):
          #                 with open(output_path, "wb") as f:
          #                     f.write(image_bytes)
          #             # else:
