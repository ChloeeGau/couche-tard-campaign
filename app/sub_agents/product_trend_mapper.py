from app.schema import Product, ProductTrendMapping, TrendMatch, Trend
...
async def load_image_into_artifact(product: str, tool_context: ToolContext, image_gcs_file_path: str) -> Optional[str]:
    ...
    if normalized_uri:
        try:
            ...
            product_photo_part = _load_gcs_image(normalized_uri, storage_client)
            ...
            logging.error(f"Failed to process GCS URI '{product_photo_filename}': {e}. Will attempt to fetch from BigQuery.", exc_info=True)
    else:
        logging.error(f"Invalid gcs image '{product_photo_filename}': Invalid path.")
        raise ValueError("Failed to load image from GCS.")
    
    try:
        # Verify the artifact exists by trying to load it.
        tool_context.load_artifact(product_photo_filename)
        logging.info(
...
async def retrieve_image_from_gcs(image_path: str, tool_context: ToolContext) -> str:
    ...
    load_image_into_artifact(source_file_name_only, tool_context, min_blob_name)
...
async def identify_product_from_image(image_path: str) -> Product:
    """
    ...
    try:
        ...
        # Fallback
        # Fallback
        from app.schema import CoreIdentifiers, Attributes, Categorization, CommercialStatus, Media, Description
        return Product(
...
async def map_product_to_trends(product: Product, image_path: str, tool_context: ToolContext) -> ProductTrendMapping:
    """
    Maps a product to relevant micro and macro trends using Gemini.
    """
    # try:
    image_path = await normalize_bucket_uri(image_path)
    ...
    tool_context.save_artifact("matching_trends", result)
    print(f"session state: {tool_context.session.state}")
    matching_trends = tool_context.session.state.get("matching_trends")
    print(f"matching trends: {matching_trends}")
    tool_context.session.state['matching_trends'] = result
    matching_trends = tool_context.session.state.get("matching_trends")
    ...
    logging.info(f"Saved matching trends for product '{product}' as artifact 'matching_trends'")
...
class ProductTrendMapper:
    def __init__(self):
        ...
        # )
      try:
          with open("app/prompts/product_trend_mapper.md", "r") as f:
