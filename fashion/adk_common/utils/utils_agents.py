...
from fashion.adk_common.dtos.generated_media import GeneratedMedia
...
from fashion.config import PROJECT_ID, LOCATION
...
async def save_to_artifact_and_render_asset(
    ...
    if save_in_artifacts:
        ...
        log_message(f"Saved file in artifacts. image_url: {asset.gcs_uri}. asset_name: {asset.filename}", Severity.INFO)

    if save_in_gcs:
        # bucket_path = f"{GOOGLE_CLOUD_BUCKET_ARTIFACTS}/{gcs_folder}" if gcs_folder else GOOGLE_CLOUD_BUCKET_ARTIFACTS
        # bucket_path = f"{GOOGLE_CLOUD_BUCKET_ARTIFACTS}"
        bucket_path = f"{GOOGLE_CLOUD_BUCKET_ARTIFACTS}"
        destination_blob_name = f"{gcs_folder}/{asset.filename}" if gcs_folder else asset.filename
        ...
        log_message(f"Saved file in GCS. image_url: {asset.gcs_uri}. asset_name: {asset.filename}", Severity.INFO)
...
def store_inline_artifact_metadata(context: CallbackContext, asset: GeneratedMedia):
...
def get_and_clear_inline_artifacts(context: CallbackContext) -> Tuple[dict, dict]:
    session_artifacts = context.state.get(SESSION_ARTIFACTS_STATE_KEY, {})
    if not isinstance(session_artifacts, dict):
        # Fallback if it was somehow initialized different than a dict
        session_artifacts = {}
    ...
    return session_artifacts, temp_artifacts



def get_or_create_unique_session_id(callback_context: CallbackContext):
    session_id = get_unique_session_id(callback_context)
    if not session_id:
...
def to_dict_recursive(obj: Any) -> Any:
    ...
    Returns:
        dict or Any: The dictionary representation of the object, or the object itself if it has no `__dict__` attribute.
    """
