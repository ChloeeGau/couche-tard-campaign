...
PROJECT_ID = "gemini-enterprise-banking-1446"
DATASET_ID = "Modern_Muse"
JSON_FILE_PATH = "/Users/andrewturner/Documents/antigravity/Davos/bq_product.json"  # Ensure this file exists in the directory
...
def get_seasonality_multiplier(product, month):
    ...
    if any(x in text for x in ['boot', 'fleece', 'sweater', 'jacket', 'coat', 'wool']):
        return 1.8 if is_winter else 0.4
    if any(x in text for x in ['sandal', 'short', 'linen', 'crop', 'sleeveless', 'swim']):
        return 1.8 if is_summer else 0.4
    if any(x in text for x in ['dress', 'skirt']):
        return 1.3 if (month in [4, 5, 6, 12]) else 0.8 
...
for plan in local_plan_cache:
    ...
    # --- MODIFIED LOGIC FOR SKU 292929 ---
    if plan['sku'] == "292929":
        # Force huge underperformance (20% to 40% of plan)
...
# 8. VERIFY RESULTS
# ---------------------------------------------------------
print("\n--- Verification: Jan 2025 Plan vs Sales (Highlighting SKU 292929) ---")
