...
*   **New Social Media Post**: If the user asks to create a new social media post, run the following steps:
    ...
    3. Call `social_media_post_agent.product_research` to generate the campaign, passing in the selected,`Product` object from step 2 and selected trend,`Trend` object from step 1, adhering to the schema defined in `retail_ops/schemas/social_media.py`.
    4. Call `social_media_post_agent.generate_keyframe_image_prompt` to generate the campaign, passing in the selected,`Product` object from step 2 and selected trend,`Trend` object from step 1, and `product_research` from step 3, adhering to the schema defined in `retail_ops/schemas/social_media.py` and returning the image prompt, video prompt and image path.
